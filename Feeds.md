...
*Hi everyone, I’m XXX and I’m the project leader of the Feeds Capsule currently.*
...
**Feeds dApp简要地说就是一款去中心化的社交应用软件，在这款应用中用户完全拥有自己数据的控制权。**

*In brief , Feeds Capsule is a decentralized social platform application where users remain in full control of their own data.*

**当初我们在立项启动开发这款去中心化应用时，主要有两个初衷：**

*When we decided to launch the development of this decentralized application, there were two original purposes:*

- **当elastOS应用成功发布給社区后，就会需要有一款以内容与流量导向为主的应用来吸引和引导社区人员使用elastOS；**
...
- **在使用亦来云基础服务之上，探索一种新型的去中心化应用和服务模式。**
  ...
  *And meanwhile, we are exploring a new type of decentralized application and service model based on Elastos infrastructures.*

**希望Feeds dApp能够持续践行和实现这两个项目初衷，同时也是立身践行亦来云项目宗旨 “You Own Your Data”, 这其中就需要亦来云社区爱好者的参与和支持。**

*Therefore, we hope Feeds Capsule will accomplish these two original purposes well, and also implement the mission of “You own your data” into reality, which requires the participation and support of Elastos community.*



### Feeds Update （Feeds现状）

**Feeds dApp在今年8月中旬以技术预览版形式完成初始版本发布。至现在为止，已经迭代了一个BugFix版本和一个新特性版本。**
...
*Compared with traditional social applications such as Twitter and Weibo, Feeds Capsule has the following most essential traits:*

- **使用去中心化ID身份验证登录后台服务。传统的社交应用使用中心化的用户账号系统或者第三方oAuth协议进行用户认证和授权。这种传统的用户身份认证方式不仅无法保证用户对自己账号信息的控制，也无法避免潜在的单点服务登录失败的问题。**

  *Feeds Capsule use Elastos DID (Decentralized ID) to sign-in and access backend services. In comparison, for traditional social application, they use the centralized account system or 3rd-party oAuth framework for authentication and authorization, in which, not only it can not be guaranteed the full control of user's own credential, but also make it impossible to avoid the potential failure of single sign-on service.*

- **由于Feeds服务的去中心化，从前端用户角度来看，拥有了对不同后台服务访问的选择权，这样有益于服务内容的多样化。**

  *With the decentralization of Feeds services,  users have freedom of choices to get access for more different Feeds services, which would be beneficial to the diversity of service contents.*

- **同时由于Feeds服务的去中心化，任何人都可以运行Feeds服务，并且用户对自己的数据有完全控制权。**

  *With the decentralization of Feeds service, it shows that the Feeds service can be run by anyone and empower them with the full control of their own data.*



## Page3
**在用户体验上，Feeds dApp其实跟Twitter、 微博等比较类似。**

*In terms of the services and the user experience, Feeds Capsule is actually similar to Twitter and Weibo.*

**在Feeds中，如果用户对某个Feed感兴趣，则用户需要先登录该Feeds所在的Feeds Service，然后再关注对应的Feeds，进而可以阅读Feeds里面的一系列微说，并基于微说回复一些评论。**
...
**从这几个图你可以看到，我们关注了几个Feeds，而这些Feeds在各自不同的Feeds服务节点上。目前这些Feeds服务节点由不同的团队或者个人根据自己的兴趣自主运营和管理。**
...
*Basic features of Feeds Capsule*

- **用户（可以是个人或者机构组织）只能在自己运营的Feeds中发布微说;**

  *User (can be individual or organization ) can only make posts in the Feeds he created;*

- **用户可以阅读已关注的Feeds中的微说和评论，同时就微说发表评论；**

  *User can read posts and comments in the Feeds being followed, and also can make comments on specific posts in that Feed;*

- **用户可以自动收到通知，让你知晓有哪些用户关注了你运营的Feeds，点赞了你发布的微说，或者在你的微说上留下了评论；**
...
- **通过ELA钱包打赏你喜欢的Feeds；**
  ...
  *Users can tip for your favorite Feeds*
...
- **支持更多的分享Feeds服务途径，除了支持主流的第三方应用途径分享，也可以通过elastOS分享Feeds服务。同时，扩展支持直接推荐和分享Feeds本身；**
  ...
  *Support more ways to share Feeds service. In addition to providing main sharing ways via 3rd-party applications, you can also share Feeds service via elastOS. And also support for direct recommendation and sharing for Feeds itself;*

- **支持私密Feeds。私密Feeds开始时可能对用户不可见，可直接通过朋友推荐请求关注，而且只有Feeds的所有者同意后才允许被关注；**
...
- **支持付费Feeds。在用户完成付费的情况下才能加入该Feeds。一般情况下，这些付费Feeds能给用户提供更有价值的创新内容；**
...
- **规划在Feeds dApp生态中引入经济激励机制，例如基于亦来云ETH侧链的ERC20 Token，通过Token奖励吸引更多用户参与，从而提升内容品质以及用户流量；**
...
- **将Feeds dApp改造成也可独立运营在Android/iOS等移动平台上的原生应用。其实目前Feeds dApp前端是仅能运行在elastOS平台的胶囊程序，还无法作为原生应用运行在Android/iOS等平台上。**
...
- **一旦elastOS工具链支持将elastOS dApps打包成为移动平台原生应用，同时elastOS运行时提供原生应用支持后，Feeds dApp也将提供Android和iOS的原生应用。这依赖于elastOS的相关工作进展，预定于今年年底提供。**
  ...
  *Once elastOS tool chain supports packaging capsules as native applications on Android/iOS platforms, and elastOS runtime provides with native application support for dApps, Feeds dApp will provide native application on Android/iOS platforms, which depends on the progress of elastOS, and would be slated on Dec. 2020 for this first release.*
